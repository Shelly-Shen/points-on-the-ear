...
//用localStorage带来的disease参数http://p3a3la3o0.bkt.clouddn.com/image/videoIcon4.jpg
...
$.ajax({
    ...
    success: function(data) {
        $(".main-class").append(
            '<p style="border: solid gray 1px" class="main-class-attention"><span class="main-class-span"><b>疾病描述:</b></span><span>' +
            data.diseaseDescription +
            '<span/></p><button class="btn am-btn-primary">查看治疗方案</button>'
        );
        $(".btn").click(function() {
            var mod = [];
            var arr = [];
            ...
            $(".main-class").append(
                '<p style="border: solid gray 1px" class="main-class-attention"><span class="main-class-span"><b>治疗方案：</b></span>' +
                data.attention +
                "</p>"
            );
            //diseaseDescription
            ...
            for (var i = 0; i < mod.length; i++) {
                $(".main-class").append(
                    '<a class="fl" href="#myvideo" name=' + vid[i] + ">" + mod[i] + "</a>"
                );
            }
            $(".main-class ").append("<span><p>配穴:</p></span>");
            for (var i = 0; i < arr.length; i++) {
                $(".main-class").append(
                    '<a class="fl" href="#myvideo" name=' + vad[i] + ">" + arr[i] + "</a>"
                );
            }
            $(".main-class").append(
                "</br><video src='+vid[0] +' width='300px' height='200px' controls='controls' id='myvideo'>"
            );
            $(".main-class a").each(function() {
                $(this).click(function() {
                    mySrc = $(this).attr("name");
